...
class Env():
    ...
    def getOdometry(self, odom):
        ...
        ### compute how much we're off target in angles ###
    ...
    def getState(self, scan):
        ...
        if min_range > min(scan_range) and min(scan_range) > 0: ###!!!
            done = True  # means collision
        ...
        if current_distance <= self.threshold_arrive:
            # done = True
            arrive = True  # means arrive at goal
    ...
    def setReward(self, done, arrive):
        ...
        reward = 500.*distance_rate  # how much we approach in this step
        self.past_distance = current_distance

        if done:  # add penalty if collision
            reward = -100.
            self.pub_cmd_vel.publish(Twist())

        if arrive: # add high reward if arriving goal
            reward = 120.
            self.pub_cmd_vel.publish(Twist())
            rospy.wait_for_service('/gazebo/delete_model') 
            self.del_model('target') # remove previous target

            # Build the target
            rospy.wait_for_service('/gazebo/spawn_sdf_model') # build a new target
            try:
    ...
    def step(self, action, past_action): # 
        ...
        state = [i / 3.5 for i in state]  # normalize 
